...
THREEx.JSARToolKit = function (opts) {
  ...
  const arRaster = new NyARRgbRaster_Canvas2D(canvasRaster)
  // FLARParam is the thing used by FLARToolKit to set camera parameters.
  // Here we create a FLARParam for images with 320x240 pixel dimensions.
  const arParam = new FLARParam(canvasRaster.width, canvasRaster.height)
  // The FLARMultiIdMarkerDetector is the actual detection engine for marker detection.
  // It detects multiple ID markers. ID markers are special markers that encode a number.
  // - 100 seems to be a zoom factor for the resulting matrix
  const arDetector = new FLARMultiIdMarkerDetector(arParam, 100)
  ...
  const tmpGlMatCam = new Float32Array(16)
...
THREEx.JSARToolKit.prototype.update = function () {
  const canvasRaster = this._canvasRaster
  const markers = this._markers
  const arRaster = this._arRaster
  const arDetector = this._arDetector
  const events = []

  const ctxRaster = canvasRaster.getContext('2d')
  ...
  const nDetected = arDetector.detectMarkerLite(arRaster, this._threshold)

  // Go through the detected markers and get their IDs and transformation matrices.
  for (let idx = 0; idx < nDetected; idx++) {
    var markerId
    // extract the markerId
    const id = arDetector.getIdMarkerData(idx)
    ...
    const eventType = markers[markerId] ? 'update' : 'create'
